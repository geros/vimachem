...
public sealed class EventRetentionJobExtremeTests {
	...
	[Fact]
	public async Task ExecuteAsync_NullServiceProvider_ShouldThrowArgumentNullException() {
		// Arrange
		var mockLogger = new Mock<ILogger<EventRetentionJob>>();

		// Act & Assert
		Assert.Throws<ArgumentNullException>(() =>
			new EventRetentionJob(null!, mockLogger.Object));
	}

	[Fact]
	public async Task ExecuteAsync_NullLogger_ShouldThrowArgumentNullException() {
		// Arrange
		var serviceProvider = new ServiceCollection().BuildServiceProvider();

		// Act & Assert
		Assert.Throws<ArgumentNullException>(() =>
			new EventRetentionJob(serviceProvider, null!));
	}
