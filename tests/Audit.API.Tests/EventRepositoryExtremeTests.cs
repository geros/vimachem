...
public sealed class EventRepositoryExtremeTests {
	...
	public async Task GetPartyEvents_ZeroOrNegativePageSize_MayCauseIssues(int pageSize) {
		...
		if (pageSize == 0) {
			// With pageSize 0, MongoDB returns empty result set (division by zero in TotalPages calculation)
			var result = await repository.GetPartyEventsAsync("party-123", 1, pageSize, CancellationToken.None);
			// TotalPages calculation: Math.Ceiling(100 / 0.0) = Infinity, cast to int = int.MaxValue or exception
			Assert.True(result.TotalPages < 0 || result.TotalPages > 100 || result.TotalCount == 100);
		} else {
			// Negative pageSize - MongoDB may throw or return unexpected results
			try {
				var result = await repository.GetPartyEventsAsync("party-123", 1, pageSize, CancellationToken.None);
				// If it succeeds, TotalPages would be negative or zero due to Math.Ceiling with negative divisor
